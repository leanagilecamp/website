...
mkdir -p output
rsync -a www output/
day=`date "+%Y%m%d"` && sed -i "s/site.js/site_$day.js/" output/www/index.html && mv output/www/site.js output/www/site_$day.js
for include in output/www/*_include.html 
do
	fichier_html=`echo $include |sed -e 's/_include//'`
	echo $fichier_html
	cat output/www/index.html | awk  -F '<!--#include page -->' 'BEGIN {RS="</html>\n"} {print $1}'  > $fichier_html
